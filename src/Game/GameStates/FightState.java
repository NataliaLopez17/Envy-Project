...
public class FightState extends InWorldState{
	...
	public void tick() {
		...
		if (WorldManager.enemyDefeated == false) {
			skill = WorldManager.enemyDefeated;
		}
		if (WorldManager.enemyDefeated == false) {
			Player.mana = 0;
		}

		//if (Player.mana < 25 && WorldManager.enemyDefeated == false) skill = false;
	...
	public void render(Graphics g) {
		...
		if(battleOver){
			...
			if(handler.getEntityManager().getPlayer().getHealth()==0){
				...
				if (enemy.name == "Jovan") {
					WorldManager.enemyDefeated = true;
					Player.mana = 100;
				}
	...
	private void setUiManager() {
		...
		//Skills
		if (WorldManager.enemyDefeated == false) {
			uiManager.addObjects(new UIImageButton(handler.getWidth() * 38/60 - 128/2, 5*handler.getHeight()/6, 128, 64, Images.Skill2, new ClickListlener() {
			...
			uiManager.addObjects(new UIImageButton(handler.getWidth() * 38/60 - 128/2, 5*handler.getHeight()/6, 128, 64, Images.Skill3, new ClickListlener() {
				@Override
				public void onClick() {
